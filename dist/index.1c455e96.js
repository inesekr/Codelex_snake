...
})({"5TkPT":[function(require,module,exports) {
"use strict";
...
module.bundle.HMR_BUNDLE_ID = "9eaae3471c455e96";
...
},{}],"crwHW":[function(require,module,exports) {
...
const appleIcon = "\uD83C\uDF4E";
const createCanvas = (canvasConfiguration)=>{
    const container = document.getElementById("game");
    const canvas = document.createElement("Canvas");
    container.appendChild(canvas);
    const { scale , cellSize  } = canvasConfiguration;
    // canvas element size in the page
    canvas.style.width = (0, _configurationDefault.default).nbCellsX * cellSize + "px";
    canvas.style.height = (0, _configurationDefault.default).nbCellsY * cellSize + "px";
    // image buffer size
    canvas.width = (0, _configurationDefault.default).nbCellsX * cellSize * scale;
    canvas.height = (0, _configurationDefault.default).nbCellsY * cellSize * scale;
    return canvas;
};
class GameUI {
    ...
    drawApples(context) {
        ...
        apples.forEach((cell)=>context.fillText(appleIcon, cellSize * scale * cell.x + cellSize, cellSize * scale * cell.y + cellSize));
    }
    drawSnake(context) {
        ...
        tail.forEach((cell)=>context.fillRect(cellSize * scale * cell.x, cellSize * scale * cell.y, cellSize * scale, cellSize * scale));
    }
    onKeyDown(event) {
        const snake = this.game.getSnake();
        switch(event.key){
...
new GameUI(createCanvas({
    cellSize: 24,
    scale: 2
}), new (0, _game.Game)());

},{"./configuration":"kyQGN","./Game":"TyEjs","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"kyQGN":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
class Game {
    ...
    update(time) {
        this.nextTick = time + (0, _configurationDefault.default).defaultSpeed;
        this.snake.move();
        switch(this.checkState()){
            ...
            case 1:
                ...
                if (this.field.isEmpty()) this.levelUp();
        }
    }
    checkState() {
        const cell = this.snake.getHead();
        // left the play area or ate itself??
        if (this.isOutside(cell) || this.snake.isTakenBySnake(cell)) // dead
        return -1;
        // ate apple?
        if (this.field.isAppleInside(cell)) return 1;
        // nothing special
        return 0;
    }
    levelUp() {
        ...
        if (this.runtimeConfiguration.level < (0, _configurationDefault.default).maxLevel) {
...
var _cell = require("./Cell");
class Snake {
    head = new (0, _cell.Cell)(2, 0);
    tail = [
        new (0, _cell.Cell)(0, 0),
    ...
    move() {
        ...
        if (this.direction === "Up") this.head = new (0, _cell.Cell)(head.x, head.y - 1);
    }
    grow(numToGrow) {
        const tailEnd = this.tail[this.tail.length - 1];
        for(let i = 0; i < numToGrow; i++){
            const newTailCell = new (0, _cell.Cell)(tailEnd.x, tailEnd.y);
